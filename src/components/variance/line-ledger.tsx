...
// The line's reconciliation ledger — the field-level Filed/Expected/
// Corrected table both variance detail pages (rule and AI) lead with. The
// table is the WHOLE line's story: every issue's diff rows render at once,
// each colored by its issue's status, so hopping between the line's
// variances never changes what's on the table — only which rows are amber.
...
export function LineLedger({
  ...
  currentId: string;
  /** The current issue's comparison snapshot; the declared-HTS fallback
   *  when re-ingestion removed the line. {} for AI findings. */
  currentDetails: Record<string, unknown>;
  line: VarianceDetail["line"];
  catalogExpected: VarianceDetail["catalogExpected"];
  ...
  const str = (k: string) =>
    typeof currentDetails[k] === "string" ? (currentDetails[k] as string) : null;
  ...
  const unitCtxs: UnitCtx[] = units.map((u) => {
    ...
    return {
      ...
      tag: {
        status: unitStatus(u),
        current: unitIds(u).includes(currentId),
      },
      details: det,
  ...
  const sourceLabel = (c: UnitCtx): string | null => {
    ...
    switch (c.type) {
      ...
      default:
        return c.type.startsWith("ai_") ? "Source: AI" : null;
    }
  };
  ...
  // the events feed's SKU scoping), and AI opens the finding's own variance
  // page, where the analyst's reasoning lives. Schedule has no page yet (a
  ...
  const sourceHref = (c: UnitCtx, label: string): string | null => {
    ...
    if (label === "Source: AI") {
      // The current finding's own chip stays plain — the reasoning is
      // already on this page.
      return c.tag.current
        ? null
        : `/variance/${c.id}${fromEntry ? "?from=entry" : ""}`;
    }
    return null;
  };
  // Clicking a row in question jumps to the variance that raised it — the
  // sidebar selection follows the URL. The current issue's rows don't link;
  // the reviewer is already on them.
  const varianceHref = (c: UnitCtx): string | undefined =>
    c.tag.current
      ? undefined
      : `/variance/${c.id}${fromEntry ? "?from=entry" : ""}`;
  ...
  const catalogDutyRows = (c: UnitCtx): DiffRow[] => {
    ...
    if (expRate !== null) {
      ...
      out.push({
        ...
        issue: tag,
        href: tag ? varianceHref(c) : undefined,
      });
    }
    if (
  ...
  if (line && catalogPair.length === 0) {
    ...
    // Several issues can stack into this one row; the click target is the
    // first still-open contributor — and nothing when the current issue is
    // among them (the row is partly this page's own question).
    const totalTarget = impactUnits.some((c) => c.tag.current)
      ? undefined
      : (impactUnits.find((c) => c.tag.status === "open") ?? impactUnits.at(0));
    rows.push({
      key: "duty-total",
      field: "Duty",
      expected:
      ...
      issue: tag,
      href: totalTarget ? varianceHref(totalTarget) : undefined,
    });
  }
    ...
    <>
      <div className="overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm">
        <Table className="[&_td]:py-3.5">
          ...
          <TableBody>
            ...
            {/* Hover is a clickability cue: only linked rows change on
                hover — a deepened tone when the row carries a status color,
                the table's default wash otherwise. Unlinked rows (the
                current issue's, context facts) hold their exact color:
                there's nowhere to go. The hover classes override TableRow's
                default via twMerge. */}
            {rows.map((row) => {
              const currentOpen =
                row.issue?.current && row.issue.status === "open";
              const hover = row.href
                ? currentOpen
              ...
              return (
                <LinkedTableRow
                  ...
                  href={row.href}
                  className={cn(
                    currentOpen && "bg-amber-50/50 dark:bg-amber-950/20",
                    row.issue?.status === "resolved" &&
      ...
      {!line ? (
        <p className="mt-3 text-sm text-muted-foreground">
          The flagged line was re-ingested and no longer exists; showing the
          facts the alert recorded when it fired.
        </p>
      ) : null}
    </>
  );
