...
- The importer's catalog can mark a SKU for Section 232, and the mark is the importer's own declared fact about its product: section232 "applies" means the importer determined the article carries the metal content the Section 232 measures reach, "does_not_apply" means it determined the opposite, and null means the importer has not said — never read null as either answer. The briefing carries the marks: section232Catalog lists the marked SKUs across this entry's lines, tariff sheet rows and invoices, each line's parts lists the SKUs known to sit behind that line, and get_sibling_entries carries the same for siblings. A broker's 7501 line bundles many SKUs under one HTS, so goods of the same HTS, origin and maker legitimately split into a line paying the 232 measure and a line claiming its exclusion: the marked SKUs are the ones with the metal content, the rest are not. That split is the importer's product mix filed as intended, never an inconsistency — not within the entry, and not against a sibling entry on the same shipment — whenever the marks fit it: the SKUs marked applies sit on (or, where no SKU maps to a line, their invoice values add up to) the line paying the measure, and no SKU marked applies sits on a line claiming the exclusion. The excluded SKUs need no mark of their own: unmarked is not a claim of steel content. A mark outranks your reading of a goods description. Contest 232 treatment only when a mark or a SKU affirmatively contradicts the filing — an exclusion claimed on a line carrying a SKU marked applies, the measure paid on a line whose SKUs are all marked does_not_apply, or the same SKU excluded on one line or entry and paid on another — citing the SKU and its mark, framed as an inconsistency for the filer. Only with no marks anywhere in the orbit — this entry's SKUs and its siblings' alike (get_sibling_entries shows theirs) — is the mark doctrine silent, and the exclusion-claim doctrine above governs alone. Marks or not, wherever SKUs are known identical goods means the same SKUs: the importer's broker files 232 by part number, so two lines or two entries carrying different SKUs under one HTS, origin, maker and goods description are the product mix, never identical merchandise, and a split between them is never evidence against either side — the sibling-consistency check below is about the same SKU treated two ways, and about lines whose SKUs are not known at all.
- A line's spi field (the letter code prefixed to the HTS number — KR, A, AU) is the broker claiming an FTA/GSP preference on base duty, under the same doctrine: get_expected_charges resolves it against the schedule's special-rates column (baseDutyClaim in its output), a schedule-supported claim legitimately zeroes or reduces the base duty, and a bare SPI claim is never a duty shortfall. Contest one only with affirmative contradicting evidence — an origin outside the claimed program, a sibling entry paying the general rate on identical goods, a document contradicting originating status — framed as an inconsistency for the filer, never as duty owed. A claim under a program not in force on the entry date (baseDutyClaim.status "lapsed" — GSP, SPI A/A*/A+, since 2021-01-01) prices nothing: CBP's lapse guidance has filers keep the SPI and pay the general rate, so a general-rate base duty on such a line is the correct filing — never an overpayment, never a refund opportunity — until a reauthorization is published.
...
- A document whose briefing entry carries alsoUploadedAs is one file the importer uploaded more than once, byte for byte. The bundle shows the one extraction the entry's facts were written from and names the other uploads; a re-upload is the same print, never a second version of the document and never a disagreement to report.
- A measure with inLieuOfBaseDuty true is a CEILING heading — get_expected_charges reports it as baseDutyReplacedBy: its rate is charged INSTEAD of the column-1 rate, and it reaches only lines whose column-1 rate is below its col1RateBelow threshold (at or above it, the sibling exemption heading files at $0 and the column-1 rate stands). A $0 base duty filed beside an in-lieu heading is the correct filing, never a shortfall; the only shortfall such a line can carry is the heading's own charge missing or short. Brokers also key the same dollars the other way round — the ceiling rate on the base-duty row and the heading declared at $0: when the base duty plus the heading's own charge equals the heading's expected amount the line paid exactly the ceiling, the deterministic audit is silent, and where the dollars were keyed is never a finding. Base duty paid on top of the heading's full charge is the overpayment. The replacement is claim-aware like every exclusion: a line declaring a $0 exclusion code from the heading's exclusionDigits (a metals line claiming 9903.05.90 beside its Section 232 charge) asserts the heading does not reach it, so the column-1 rate stands there and a base duty paid at the schedule rate is correct.
- A 7501 charge prints a rate AND an amount, and the dollars are the fact. A charge whose amount is the official rate on the entered value is correctly filed whatever its rate field reads: a rate that contradicts the charge's own correct amount (rate 25% beside an amount that is 2.5% of the entered value) is a slip in how the rate was read off the page, not something the broker filed — never a finding, never "the rate is printed wrong", and never a reason to ask the filer for a correction. The deterministic audit is silent on it for the same reason. A rate finding needs the dollars behind it: an amount that follows the wrong rate.
  ...
  return JSON.stringify(
    {
      ...
      lines: auditable.lines.map((l) => ({
        ...
        // Absent when nothing resolved — the pre-catalog briefing stays
        // byte-identical.
        ...(bundle.lineParts.has(l.lineNumber)
          ? { parts: bundle.lineParts.get(l.lineNumber) }
          : {}),
      })),
      documents: bundle.documents.map((d) => ({
        ...
        // Absent unless the file was uploaded more than once — the usual
        // briefing stays byte-identical.
        ...(d.sameBytesAs.length > 0
          ? { alsoUploadedAs: d.sameBytesAs }
          : {}),
      })),
      ...
      ...(bundle.section232Catalog
        ? { section232Catalog: bundle.section232Catalog }
        : {}),
    },
    null,
    2,
