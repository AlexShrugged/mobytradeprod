...
// On approve, the reviewer's CONFIRMED dates — exactly the fields the
// structured feed never carries. The sail-clause highlights are evidence
// for choosing these values, never auto-applied.
...
export async function PATCH(
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      // Measure windows changed — re-derive persisted audit findings
      // (expected charges self-heal on read; alerts do not).
      const audit = await sweepAudits(tx, orgId);
